...
class PythonSQLValidator:
    ...
    def _fix_nested_quotes(self, code: str) -> str:
        """Fix meta-programming quote issues"""
        # Handle triple quote conflicts
        def fix_inner_quotes(match):
            content = match.group(1)
            # Replace inner triple double quotes with single quotes
            content = content.replace('"""', "'''")
            return f"'''{content}'''"
        
        # Fix triple single quotes containing triple double quotes
        pattern = r"'''(.*?)'''"
        code = re.sub(pattern, fix_inner_quotes, code, flags=re.DOTALL)
...
@click.option('--fix', is_flag=True, default=True, help='Auto-fix and save')
@click.option('--strict', is_flag=True, default=True, help='Enable data science checks')
@click.option('--verbose', '-v', is_flag=True, default=True, help='Detailed output')
...
def validate(file_path, language, fix, strict, verbose, backup):
    ...
    print(f"Validating {file_path} as {lang_enum.value if lang_enum else 'auto-detected'}...")
    print(f"fix: {fix}, strict: {strict}, verbose: {verbose}, backup: {backup}")
    print("-" * 50)
