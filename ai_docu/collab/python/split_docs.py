...
def main(source: Path, num_splits: int, output: Path, compress: bool):
    ...
    python split_docs.py -s /path/to/documents -n 3 -o /path/to/packages
    """
    ...
    try:
        ...
        if compress:
            ...
            click.echo(f"4. Team lead uses join_docs.py to merge results")
        else:
            click.echo(f"1. Compress packages: python split_docs.py -s {source} -n {num_splits} -c")
