...
def _clean_mermaid_specific(code: str) -> str:
    ...
    for i, line in enumerate(lines):
        ...
        # Look for proper mermaid diagram start (keyword + additional content)
        for keyword in KNOWN_MERMAID_TYPES:
            if stripped_line.startswith(keyword) and len(stripped_line) > len(keyword):
                start_idx = i
                break
        if start_idx != -1:
            break
    ...
    if start_idx != -1:
        # Extract from the proper start line onwards
