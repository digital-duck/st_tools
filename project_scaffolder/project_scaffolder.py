...
class ProjectScaffolder:
    ...
    def parse_text_structure(self, text_content: str) -> Dict:
        ...
        for line_num, line in enumerate(lines):
            ...
            # Extract item name and comment first
            ...
            # print(f"Line {line_num:2d}: Level {indent_level} | '{item_name}' | Stack before: {[p for p in path_stack]}")
            
            # Adjust path stack based on indentation
            # This is the critical part - we need to truncate the stack correctly
            ...
            if indent_level == 0:
                ...
                for path_part in path_stack:
                    if path_part in current and current[path_part].get('children') is not None:
            ...
            # Update path stack for directories
            if item_info['type'] == 'directory':
                path_stack.append(item_info['name'])
            
            # Debug - uncomment to see what's happening
            # print(f"         Stack after: {[p for p in path_stack]}")
        
        return root
    
    def _calculate_indent_level(self, line: str) -> int:
        """Calculate indentation level from tree structure"""
        # Strip the line to work with the tree structure
        stripped = line.lstrip()
        
        # Calculate how much was stripped (total indentation)
        total_indent = len(line) - len(stripped)
        
        # Count actual tree depth by looking for tree characters
        # Standard tree notation uses 4 characters per level
        # "├── " or "└── " or "│   " or "    "
        ...
        prefix = line[:content_match.start(1)]
        
        # Count tree levels more carefully
        # Each level should be either:
        # - 4 spaces "    "
        # - vertical bar + 3 spaces "│   "  
        # - branch + 2 dashes + space "├── " or "└── "
...
@cli.command()
@click.argument('input_file', type=click.Path(exists=True))
@click.option('--show-levels', is_flag=True, help='Show indent levels for debugging')
def debug_parse(input_file, show_levels):
    """Debug text parsing - show how lines are interpreted"""
    scaffolder = ProjectScaffolder()
    ...
    click.echo("Debug parsing output:")
    click.echo("="*80)
    ...
    for line_num, line in enumerate(lines):
        ...
        item_name, comment = scaffolder._extract_item_info(line)
        
        if not item_name:
            continue
            
        indent_level = scaffolder._calculate_indent_level(line)
        
        # Simulate path stack logic
        while len(path_stack) > indent_level:
            path_stack.pop()
        ...
        # Show the path
        current_path = '/'.join(path_stack + [clean_name])
        
        click.echo(f"Line {line_num:2d}: Level {indent_level} | Path: {current_path} | Comment: {comment}")
        
        if show_levels:
            click.echo(f"         Raw: '{line}'")
            click.echo(f"         Stack before: {path_stack}")
        ...
        if show_levels:
            click.echo(f"         Stack after:  {path_stack}")
            click.echo("")
    
    click.echo("="*80)
    
    # Parse and show resulting structure
    structure = scaffolder.parse_text_structure(text_content)
    text_output = scaffolder.structure_to_text(structure)
